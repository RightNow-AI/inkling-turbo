...
| `tune_sm80_a100.json` | A100 SXM4 40GB **claimed, unverifiable**: the file records no device, capability or torch version, because it predates those fields being written | Tile sweep, three configurations, one sample per cell. Parity-gated, but the gate ran `seqlen_q == seqlen_k` at `Hq == Hkv` while the decode cases were timed at `T_q=1` against `T_k=65536` at `Hq=64` over `Hkv=8`, and the generic kernel was wrong on both of those axes | The percentages it backed are **withdrawn, and as of 2026-07-25 refuted by measurement**: `validate_a100x1_s32_packgqa/` re-ran the sweep on verified Ampere with five interleaved rounds and found the headline figure had **the wrong sign**, `tile_n=64` ahead of `tile_n=32` by 9.7% on batch-1 decode at 64K where this file's numbers put `tile_n=32` ahead by 10.1%. Do not read the cross-session deltas between this file and `validate_a100x1_s31/` as noise: the kernel differed between every pair of those runs. [regression-ampere-tile-sweep.md](../regression-ampere-tile-sweep.md). The `tile_n=128` collapse reproduces in both runs and survives. The identical parity max diff across all three configurations, `:4`, `:13`, `:22`, is now explained rather than merely noted: the output is bf16, so one ULP is ~6e-3 relative against a ~1e-6 reduction-order perturbation, and the metric saturates on output quantization |
...
| `validate_a100x1_s31/` | A100-SXM4-40GB, `sm_80` capability (8,0) **asserted**, Modal, 0.216 h, $0.99, 2026-07-25 | The first correctness result on Ampere for anything other than single-sequence full prefill. Nine steps: `parity_rel_chunked_decode` 7/7 with per-case power 6.7x to 37.7x `TOL_MEAN`, `parity_rel_bias_coverage` 6/6, `parity_rel_varlen_batch` 11/12, `parity_fa4_rel` 3/3 on our arm, `parity_qkvr_prep` 5/5, and the tile sweep. Includes `run.log`, the full 62 KB transcript | **Strong.** The Ampere support claim's shape-family and batch-shape scope, the scope of the Ampere support claim before the pack_gqa fix, the A100 hourly rate, and a `vllm_flash_attn` `mDynamicCausal` failure on `sm_80` that is upstream and not ours. Three steps are labelled FAIL and two of those are not failures of this project's kernel; the directory README says which |
| `validate_a100x1_s32_packgqa/` | A100-SXM4-40GB, `sm_80` capability (8,0) **asserted**, Modal, 0.14 h, $0.61, 2026-07-25 | The `pack_gqa` guard on Ampere and the repeat-enabled tile sweep. `parity_rel_varlen_batch` **12/12**, up from 11/12 in s31, with `single_m_tail_chunked` at max 4.8828e-04 against 9.2529e-02 before. `parity_rel_chunked_decode` 7/7, coverage 6/6. The sweep ran 5 interleaved rounds per config and kept every raw sample | **Strong.** That the guard covers `sm_80` and not only `sm_120`, and the **refutation** of the withdrawn tile percentages: `tile_n=64` beats `tile_n=32` by 9.7% on batch-1 decode at 64K, where the published figure claimed `tile_n=32` ahead by 10.1%, so the sign was wrong. Also supersedes s31's "too noisy to rank" reading, which compared single samples across two different kernels |
