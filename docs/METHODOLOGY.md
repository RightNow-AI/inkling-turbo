...
The full integration gate is stricter than the per-op gate. It requires the fixed prompt suite and batched-output versus batch-1 consistency before a kernel is considered complete at the model level. Until those fields are populated, the [kernel ledger](../LEDGER.md#kernel-gates-ncu-90-of-binding-roofline) remains `null`.
...
| Multi-GPU serving run | End-to-end throughput and latency requires matched stock and turbo checkpoint, quantization, hardware, workload, and SLO. Results are pending. [Ledger](../LEDGER.md) |
...
The local hardware and instruction-set caveats are recorded in [phase 0](../journal/phase0.md#local-tier) and [BLOCKERS.md](../BLOCKERS.md#b3--sm_120-vs-sm_100-fp4-mma-instruction-set-open-research).
...
The current Hopper port remains in final debug. A flight reached near-plain-attention timing while producing wrong output, so it is recorded as a failed kernel rather than a win. [Ledger entry](../LEDGER.md#spend)
...
Blackwell capacity and final multi-GPU validation remain pending under the [current execution plan](../BLOCKERS.md#owner-decision-2026-07-19-lambda-only-lean-finish-plan).
