...
class FlashAttentionForwardBase:
    ...
    def _setup_attributes(self):
        ...
        if self.has_bias:
            # bias tile (tile_m, tile_n): same async atom/vec as K, cols major
            tBias_dim_1 = self.tile_n // async_copy_elems
            tBias_layout = cute.make_ordered_layout(
                (self.num_producer_threads // tBias_dim_1, tBias_dim_1),
                order=(1, 0),
            )
            self.sBias_layout = cute.make_ordered_layout(
                (self.tile_m, self.tile_n), order=(1, 0)
            )
            self.gmem_tiled_copy_Bias = cute.make_tiled_copy_tv(
                atom_async_copy, tBias_layout, vQKV_layout
            )
        else:
...
class FlashAttentionForwardSm80(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        self.use_tma_O = False  # sm80-family kernel: TMA-O never configured here (sm_90 has its own class); sm_120 shim leaves arch=sm_120 which wrongly enabled it
        ...
        softmax_scale_log2, softmax_scale = utils.compute_softmax_scale_log2(softmax_scale, self.score_mod if self.score_mod is not None else (True if self.has_bias else None))
    ...
    def kernel(
        ...
        if const_expr(mBias is not None):
            if const_expr(not seqlen.has_cu_seqlens_q):
                mBias_sliced = mBias[None, None, num_head, batch_size]
            else:
                mBias_sliced = cute.domain_offset(
                    (seqlen.offset_q, 0), mBias[None, None, num_head]
                )
            # head-slice offset (num_head*padded) is 16B-aligned by contract
            # (padded % 128 == 0) but unprovable: rebuild iterator with
            # declared alignment (pack_gqa.py:181 idiom).
            ...
            mBias_cur = assume_tensor_aligned(cute.make_tensor(
                ...
                cute.make_layout(
                    mBias_sliced.shape,
                    stride=mBias_sliced.layout.stride,
                ),
            ))
        else:
            mBias_cur = None
        ...
        if const_expr(self.has_bias and mBias is not None):
            ...
            # Dynamic col shift moved into the TILE INDEX (shift is always a
            # multiple of tile_n since padded % 128 == 0): tile (m, n) of the
            # attention grid reads bias col-tile n + bias_tile_shift. Pure
            # load_K idiom -> static strides, no pointer-alignment fight.
