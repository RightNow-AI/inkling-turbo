...
| `parity_rel_varlen_batch` | **1 of 12**, an illegal memory access | more than one sequence in the batch. **Open defect** |
...
- RTX 5090 numbers are relative only. That machine is power-capped and on WDDM, and those timings live in the journal with no JSON artifact. The correctness gates run there on 2026-07-25 are also journal-only, for a different reason: the three JSONs the session committed under `journal/remote/local_sm120_s29/` are zero-byte files.
...
| A100 (`sm_80`) | Working per-op at `seqlen_q == seqlen_k`, single-sequence, and the only attention kernel that runs | Parity 3/3 green, all three cases `seqlen_q == seqlen_k` and all three single-sequence. Day-0 cannot run here at all. The checkpoint does not fit on A100, so this is a kernel result, not a deployment. **The tile-tuning percentages are withdrawn**, see the row in [Everything else that was gated](#everything-else-that-was-gated). The generic kernel carried the same shift specialisation `sm_90` did; it is fixed in the tree at `b5f0f7e`, that fix has now run on `sm_120` but **no A100 has been available since, so on this architecture the fix is unrun and decode correctness is untested**. **Not a serving claim**: multi-sequence varlen batching on this same file faults on `sm_120`, so it is *expected* to fault here, unconfirmed. [The defect](journal/regression-sm120-varlen-illegal-address.md). |
| RTX 5090 (`sm_120`) | Working per-op on single-sequence prefill, decode and chunked prefill. Multi-sequence batching is broken | The generic kernel's corrected shear shift has now executed here, the first time on any silicon: `parity_rel_chunked_decode` 7/7 with per-case signal 7.5x to 37.8x above tolerance, `parity_fa4_rel` 3/3, `parity_rel_bias_coverage` 6/6, `parity_qkvr_prep` 5/5, all **journal-only** because the committed JSONs are zero bytes. See [What the local `sm_120` session established](#what-the-local-sm_120-session-established). `parity_rel_varlen_batch` scores **1/12** with an illegal memory access, which is the open defect. Timings 2% to 10% faster than day-0 on that machine, relative only, journal-recorded, no JSON artifact. The local headroom is structurally smaller than Hopper's. Same generic kernel as A100, so the A100 inherits the varlen expectation but not the green gates. |
